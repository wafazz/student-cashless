...
export default function Subscription({ school, packages, payments }: Props) {
    ...
    const selectPackage = (pkg: SubscriptionPackage) => {
        if (pkg.price === 0) return; // Trial can't be self-subscribed if already used
    ...
    return (
        <SchoolLayout title="Subscription">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                {packages.map(pkg => (
                    <div key={pkg.id}
                        onClick={() => !hasPending && pkg.price > 0 && selectPackage(pkg)}
                        className={`bg-white rounded-2xl p-6 border-2 transition-all ${
                            selectedPkg === pkg.id ? 'border-teal-500 shadow-md' :
                            hasPending || pkg.price === 0 ? 'border-gray-100 opacity-60' : 'border-gray-100 hover:border-teal-300 cursor-pointer'
                        }`}>
            ...
            {selectedPkg && !hasPending && (
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                    ...
                    <p className="text-sm text-gray-500 mb-4">
                        Make payment of <span className="font-bold text-teal-600">RM {Number(packages.find(p => p.id === selectedPkg)?.price || 0).toFixed(2)}</span> to our company account, then upload the receipt below.
                    </p>
